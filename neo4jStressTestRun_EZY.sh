...
for i in `seq 1 3`; do
   ...
   for i in `seq 1 10`; do
      echoi "   Starting ./neo4jStressTest_EZY.py $neoUrl $loopNums $instanceType "
      nohup ./neo4jStressTest_EZY.py $neoUrl $loopNums $instanceType $i  >> $myLOGFILE 2>&1 &
      pids="$pids $!"
   ...
   sleep 100
   echoi Killall
   #killall neo4jStressTest_EZY.py
   kill $pids
